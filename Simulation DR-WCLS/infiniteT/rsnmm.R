...
sim_wc <- function(n = 100, tmax = 30, M = 1000, high_d = 20,
  ...
  R-WCLS-fitter = function(d = d, K = 5, ml_method = "random forest"){
  ...
  DR-WCLS-fitter = function(d = d, K = 5, large_T = FALSE, ml_method = "random forest",
  ...
  out <- foreach(m = 1:M, .combine = "rbind") %dopar% {
    ...
    # 
    # beta_ML = R-WCLS-fitter(d, K = 5, ml_method = "random forest")
    
    beta_DR = DR-WCLS-fitter(d,K = 5, large_T = TRUE, ml_method = "random forest")
