---
title: "Step count with missing values"
...
gee_result_step = geeglm(as.formula(paste0("STEP_COUNT ~ week_category_new_c + study_week + ", Control_var)), 
                         data = data %>% drop_na(STEP_COUNT,all_of(Cov_set))  ,weights = w*r_w, id = UserID, scale.fix = T)
...
```{r}
fit_Step = geeglm(STEP_COUNT_minus ~ I(week_category_new - pn), id= UserID, weights = weights*r_weight, data = data, scale.fix = T )
...
```{r}
fit_Step = geeglm(STEP_COUNT_minus ~ I(week_category_new - pn)* STEP_COUNTpred_c, id= UserID, weights = weights*r_weight,
